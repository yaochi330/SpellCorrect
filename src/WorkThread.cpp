...
void WorkThread::run() {
	while (true) {
		...
		std::cout << _task._keyword << std::endl;//debug
...
void send(std::priority_queue<word_edit_freq, std::vector<word_edit_freq>, cmp> &q, const Task &task) {
	...
	for (int i = 0; (i != RETNUM) && !q.empty(); ++i) {
		send_content += trans.gbk_to_utf8(q.top().word);
		send_content += "\n";
		q.pop();
	}
...
void WorkThread::query(const std::string &word) {
	...
	for (iter = word_dict.begin(); iter != word_dict.end(); ++iter) {
		word_edit_freq wef;
		wef.word = iter->first;
		EditDistance edit_dis(iter->first, trans.utf8_to_gbk(word));
